...
def first_part():
    ...
    st.write(
        "Hello, we are Ilayda, Jan and Maximilian, the Chest X-Ray Classifiaction Team. Subsequently we will introduce our project and motivation.")
    st.write(
    ...
    st.write(
        "To make sure that you are dressed approriate to prevent pneumonia, you can check today`s weather by choosing your city below:")
...
def second_part():
    ...
    st.write(
        "Probably you have not detected it, but this person has a dangerous pneumonia, highlighted on the appeared picture above.")

    # Motivation
    st.write(
        "As demonstrated, it is not possible to identify pneumonia as a non medical. Even physicians sometimes fail in recognising dangerous lung deseases.")
    st.write(
        "Therefore we want to create a machine learning based model as a possible solution. This model will be capable of identifying pneumonia based on X-Rays.")
    st.write(
        "To state how important the recognition of lung deseases is, we provide a chart showing the number of deaths caused by pneumonia in Germany per year:")

    # Chart
    st.image("pneumonia_chart.PNG",
             caption="Deaths caused by pneumonia in Germany between 1998 and 2019; Source: Federal Statistical Office of Germany",
